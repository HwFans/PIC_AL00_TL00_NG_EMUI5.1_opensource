...
/*lint -save -e753 -specific(-e753) -e528 -specific(-e528)*/
...
/*lint -e21 -e846 -e514 -e528 -e708 -e753 -e778 -e866 -e84*/
...
MODULE_PARM_DESC(debug_online_vactive, "hisi online vactive debug");
...
MODULE_PARM_DESC(debug_ovl_mediacommon_composer, "hisi overlay media common composer debug");
...
static int hisi_fb_open_mediacommon(struct fb_info *info)
{
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970) {
		...
		sr_count = (hisifd->media_common_composer_sr_refcount)++;
		if (!sr_count) {
...
static int hisi_fb_release_mediacommon(struct fb_info *info)
{
	struct hisi_fb_data_type *hisifd = NULL;//lint !e838
	struct fb_info *fbi;
	int sr_count;
...
static int hisifb_dss_mmbuf_alloc(struct fb_info *info, void __user *argp)
{
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970_ES) {
		mmbuf_size_max = MMBUF_SIZE_MAX_ES;
	}
#endif
	/*lint -e574 -e737*/
...
static int hisifb_dss_get_platform_type(struct fb_info *info, void __user *argp)
{
	...
	if (g_dss_version_tag == FB_ACCEL_KIRIN970_ES) {
		type = FB_ACCEL_KIRIN970_ES | FB_ACCEL_PLATFORM_TYPE_FPGA;
	}
...
static int hisi_fb_register(struct hisi_fb_data_type *hisifd)
{
	...
	BUG_ON(panel_info == NULL);//lint !e730
	...
	if (hisifd->index == PRIMARY_PANEL_IDX) {
		...
		hisifd->set_fastboot_fnc = NULL;
		hisifd->open_sub_fnc = hisi_fb_open_mediacommon;
		hisifd->release_sub_fnc = hisi_fb_release_mediacommon;
		hisifd->hpd_open_sub_fnc = NULL;
		hisifd->hpd_release_sub_fnc = NULL;
		hisifd->lp_fnc = NULL;
		hisifd->esd_fnc = NULL;
...
static int hisi_fb_probe(struct platform_device *pdev)
{
	...
	if (!hisi_fb_resource_initialized) {
		...
		if (!hisifd_irq_dptx) {
			HISI_FB_ERR("failed to get hisifd_irq_dptx resource.\n");
			return -ENXIO;
		}

		hisifd_irq_mdc = irq_of_parse_and_map(np, 6);
		if (!hisifd_irq_mdc) {
			HISI_FB_ERR("failed to get hisifd_irq_mdc resource.\n");
			return -ENXIO;
		}
	#endif
		...
		hisifd_media_crg_es_base = of_iomap(np, 8);
		if (!hisifd_media_crg_es_base) {
			HISI_FB_ERR("failed to get hisifd_media_crg_es_base resource.\n");
			return -ENXIO;
		}

		hisifd_dp_base = of_iomap(np, 9);
		if (!hisifd_dp_base) {
			HISI_FB_ERR("failed to get hisifd_dp_base resource.\n");
			return -ENXIO;
		}

		hisifd_media_common_base = of_iomap(np, 10);
		if (!hisifd_media_common_base) {
			HISI_FB_ERR("failed to get hisifd_media_common_base resource.\n");
			return -ENXIO;
		}
		...
		if (ret != 0) {
			HISI_FB_ERR("failed to get dss_clk_gate_dpctrl_16m resource! ret=%d.\n", ret);
			return -ENXIO;
		}
		ret = of_property_read_string_index(np, "clock-names", 15, &g_dss_pclk_gate_dpctrl_name); //pclk
		if (ret != 0) {
			HISI_FB_ERR("failed to get dss_pclk_gate_dpctrl resource! ret=%d.\n", ret);
			return -ENXIO;
		}
		ret = of_property_read_string_index(np, "clock-names", 16, &g_dss_aclk_dpctrl_name); //ack
		if (ret != 0) {
			HISI_FB_ERR("failed to get dss_aclk_gate_dpctrl resource! ret=%d.\n", ret);
			return -ENXIO;
		}
#endif
