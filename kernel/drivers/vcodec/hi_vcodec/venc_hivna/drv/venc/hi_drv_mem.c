...
HI_S32 DRV_MEM_KAlloc(const HI_CHAR* bufName, const HI_CHAR *zone_name, MEM_BUFFER_S *psMBuf)
{
	 ...
	 HI_VOID *virt_addr = HI_NULL;
        ...
        psMBuf->u32StartPhyAddr = __pa(virt_addr);/*lint !e648 !e834 !e712*/

        snprintf(g_venc_mem_node[i].node_name, MAX_MEM_NAME_LEN, bufName);/*lint !e712 !e747*/
        g_venc_mem_node[i].node_name[MAX_MEM_NAME_LEN-1] = '\0';

        snprintf(g_venc_mem_node[i].zone_name, MAX_MEM_NAME_LEN, zone_name);/*lint !e712 !e747*/
...
HI_S32 DRV_MEM_KFree(const MEM_BUFFER_S *psMBuf)
{
        ...
        if (HI_NULL == psMBuf || psMBuf->pStartVirAddr == HI_NULL || psMBuf->u32StartPhyAddr == 0) {
                HI_ERR_VENC("%s, failed to check arguments!\n", __func__);
                goto err_exit;
        }
