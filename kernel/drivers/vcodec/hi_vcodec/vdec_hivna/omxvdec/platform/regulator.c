...
static HI_S32 VDEC_GetDtsConfigInfo(struct device *dev, VFMW_DTS_CONFIG_S *pDtsConfig)
{
    ...
    if (0 == pDtsConfig->VdecIrqNumNorm)
    {
    ...
    if (0 == pDtsConfig->VdecIrqNumProt)
    {
    ...
    if (0 == pDtsConfig->VdecIrqNumSafe)
    {
    ...
    printk(KERN_CRIT "%s VdhRegBaseAddr:%x\n", __func__, pDtsConfig->VdhRegBaseAddr);
#ifdef HIVDEC_SMMU_SUPPORT
