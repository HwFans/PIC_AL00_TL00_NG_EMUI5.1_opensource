...
#define POWER_VOTE_OFF_STATUS	0x55555555
#define POWER_VOTE_ON_UNIT		0xa
#define POWER_VOTE_OFF_UNIT		0x5
#define POWER_VOTE_UNIT_MASK		0xf
...
{
#ifdef HISEE_POWER_MANAGE_PRO
	g_vote_record_method = HISEE_POWER_VOTE_RECORD_PRO;
#endif

#ifdef HISEE_POWER_MANAGE_COUNT
...
static void hisee_set_power_vote_status(u32 vote_process, hisee_power_cmd power_cmd)
{
	u32 shift = 0;

	if (HISEE_POWER_VOTE_RECORD_PRO == g_vote_record_method) {
		/* the 2 means every vote status unit is 4 bits */
		shift = (vote_process - COS_PROCESS_WALLET) << (u32)2;
		g_power_vote_status.value &= ~((u32)POWER_VOTE_UNIT_MASK << shift);

		if (HISEE_POWER_CMD_ON == power_cmd)
			g_power_vote_status.value |= (u32)POWER_VOTE_ON_UNIT << shift;
		else
			g_power_vote_status.value |= (u32)POWER_VOTE_OFF_UNIT << shift;
...
{
	if (HISEE_POWER_VOTE_RECORD_PRO == g_vote_record_method) {
		if ((POWER_VOTE_OFF_STATUS != g_power_vote_status.value)) {/* 1: hisee reset, 0: hisee power on */
				...
				unsigned int op_cosid, hisee_power_cmd power_cmd)
{
	...
	if ((HISEE_POWER_STATUS_OFF == power_status) &&
		...
		if (HISEE_OK != ret)
			...
			hisee_set_power_vote_status(vote_process, power_cmd);
		}
		...
		if (HISEE_POWER_STATUS_OFF == power_status) {
			...
			if (HISEE_OK != ret) {
				...
				hisee_set_power_vote_status(vote_process, HISEE_POWER_CMD_ON);
...
int hisee_get_cosid_processid(void *buf, unsigned int *cos_id, unsigned int *process_id)
{
	...
	/* FIXME: support both cos_id and process_id */
	...
	if (NULL == buf) {
		...
		*process_id = MAX_POWER_PROCESS_ID;
		/* FIXME: Add count nethod check here */
...
int hisee_poweron_timeout_func(void *buf, int para)
{
	...
	if (0 == g_unhandled_timer_cnt) {
		...
		if (HISEE_OK != ret) {
			int ret_tmp;
			/* FIXME: CHECK IF THIS OFF IS NECESSARY  */
