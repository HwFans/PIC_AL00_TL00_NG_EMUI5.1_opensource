...
static int check_new_cosimage(unsigned int cos_id, int *is_new_cosimage)
{
...
static int cos_upgrade_image_read(unsigned int cos_id, hisee_img_file_type img_type)
{
	...
	if (g_platform_id == HISEE_PLATFORM_3670_CS) {
		timeout = (unsigned int)HISEE_FPGA_ATF_COS_TIMEOUT;
	}
	ret = send_smc_process(p_message_header, buff_phy, image_size,
	...
	if (g_platform_id == HISEE_PLATFORM_3670_CS) {
		if (ret == HISEE_OK && cos_id >= COS_IMG_ID_3)
			ret = hisee_encos_write((buff_virt + HISEE_ATF_MESSAGE_HEADER_LEN), image_size, cos_id);
	}
#endif
	dma_free_coherent(g_hisee_data.cma_device, (unsigned long)HISEE_SHARE_BUFF_SIZE, buff_virt, buff_phy);
...
upgrade_bypass:
	...
	if (ret != HISEE_OK)
		pr_err("%s() write rpmb failed\n", __func__);
	else
		pr_err("%s() write rpmb success\n", __func__);
