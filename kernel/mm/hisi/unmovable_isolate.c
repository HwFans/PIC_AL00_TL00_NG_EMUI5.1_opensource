...
{
	int ret = 0;
	unsigned long totalram_mbytes;
	totalram_mbytes = totalram_pages * PAGE_SIZE / SZ_1M;
	if (totalram_mbytes <= MEMSIZE_2G_in_MB)
		ret = 2;
	else if (totalram_mbytes <= MEMSIZE_3G_in_MB)
		ret = 3;
	else
...
int get_enhanced_reserve_size(void)
{
	...
	if (ddr_size!= 0 && size_cmdline_reserve[ddr_size - MEMSIZE_INDEX_OFFSET] != -1)
		enhanced_reserve_size =
		    size_cmdline_reserve[ddr_size - MEMSIZE_INDEX_OFFSET] / PAGE_SIZE / pageblock_nr_pages;
	else
...
static long long get_unmovable_isolate_blocks (int unmovable_isolate_type)
{
	...
	if (is_unmovable_isolate1(unmovable_isolate_type)) {
		if (ddr_size!= 0 && size_cmdline_ui1[ddr_size - MEMSIZE_INDEX_OFFSET] != -1)
			ui_blocks =
			    size_cmdline_ui1[ddr_size - MEMSIZE_INDEX_OFFSET] / PAGE_SIZE / pageblock_nr_pages;
		else
			ui_blocks = UNMOVABLE_ISOLATE1_SIZE_BLOCKS;
	} else if (is_unmovable_isolate2(unmovable_isolate_type)) {
		if (ddr_size!= 0 && size_cmdline_ui2[ddr_size - MEMSIZE_INDEX_OFFSET] != -1)
			ui_blocks =
			    size_cmdline_ui2[ddr_size - MEMSIZE_INDEX_OFFSET] / PAGE_SIZE / pageblock_nr_pages;
		else
